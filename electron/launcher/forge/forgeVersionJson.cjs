...
async function buildLoaderConfig({ installerPath, mcVersion, loaderName, versionSuffix, librariesDir, instanceRoot, onProgress, versionJson: providedVersionJson, installProfile: providedInstallProfile, javaPath }) {
  ...
  // Detect whether this loader uses the new client jar mechanism (srg/extra +
  // neoforge-client) or the old one (--fml.mcpVersion + binarypatcher).
  // Modern NeoForge (20.4+) and modern Forge (1.21+) both use --fml.neoFormVersion
  // and FML's ProductionClientProvider which needs the three jars generated by
  // the installer.  Legacy Forge (1.20.x) uses --fml.mcpVersion + binarypatcher.
  const usesNeoForm = () => {
  ...
  // NeoForge (20.5+) / modern Forge (1.21+) by --fml.neoFormVersion. Pick whichever
  // the version json advertises so we can verify/bundle the right client jars.
  const findClientKey = () => {
    const a = Array.isArray(versionJson.arguments?.game) ? versionJson.arguments.game : []
  ...
  // Modern NeoForge (20.4+) / modern Forge (1.21+): FML's ProductionClientProvider
  // resolves ALL of these jars itself via -DlibraryDirectory, so none go on the
  // classpath:
  //   1. client-<mc>-<neoForm>-srg.jar
  //   2. client-<mc>-<neoForm>-extra.jar    (net.minecraft:client)
  //   3. <loader>-<ver>-client.jar           (loader-specific:client)
  // None of them are published to the Maven repos — they are only generated by
  // running the installer. If any is missing we must fall through to the
  // installer fallback below, otherwise mod loading fails with
