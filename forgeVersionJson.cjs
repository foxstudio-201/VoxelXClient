...
// Complete loader version JSON from a CurseForge minecraftinstance.json.
function readVersionJsonFromInstance(instanceJsonPath) {
  try {
    const data = JSON.parse(fs.readFileSync(instanceJsonPath, 'utf8'))
    const raw = data?.baseModLoader?.versionJson
    if (typeof raw === 'string') { try { return JSON.parse(raw) } catch {} }
    else if (raw && typeof raw === 'object') return raw
  } catch {}
  return null
}

function readInstallProfileFromInstance(instanceJsonPath) {
  try {
    const data = JSON.parse(fs.readFileSync(instanceJsonPath, 'utf8'))
    const raw = data?.baseModLoader?.installProfileJson
    if (typeof raw === 'string') { try { return JSON.parse(raw) } catch {} }
    else if (raw && typeof raw === 'object') return raw
  } catch {}
  return null
...
/**
 * Build a complete loader config from an installer jar (or a CurseForge
 * instance's baseModLoader.versionJson) — without running the installer.
 *
 * @returns {null | { versionJson, installProfile, libList, librariesDir, versionId, mainClass, jvmArgs, gameArgs }}
 */
async function buildLoaderConfig({ installerPath, mcVersion, loaderName, versionSuffix, librariesDir, instanceRoot, onProgress, versionJson: providedVersionJson, installProfile: providedInstallProfile, javaPath }) {
  ...
  // Forge/legacy-neo identify the client jar by --fml.mcpVersion; modern
  // NeoForge (20.5+) by --fml.neoFormVersion. Pick whichever the version
  // json advertises so we can verify/bundle the right client jars.
  const findClientKey = () => {
    const a = Array.isArray(versionJson.arguments?.game) ? versionJson.arguments.game : []
    const token = isModernNeo ? '--fml.neoFormVersion' : '--fml.mcpVersion'
  ...
  if (needsClientJars && !isModernNeo) {
    // Only the extra jar belongs on the classpath: FML already locates the
    // srg client jar itself via -DlibraryDirectory + --fml.mcpVersion, and
    // putting it on -cp registers it twice (module "client" from the
    // filename, module "minecraft" from its mods.toml) → split package crash.
    if (mcClientDir) {
  ...
  // Modern NeoForge (20.4+): FML's ProductionClientProvider resolves ALL of
  // these three jars itself via -DlibraryDirectory, so none go on the classpath:
  //   1. client-<mc>-<neoForm>-srg.jar
  //   2. client-<mc>-<neoForm>-extra.jar    (net.minecraft:client)
  //   3. neoforge-<ver>-client.jar           (net.neoforged:neoforge:client)
  // None of them are published to the Maven repos — they are only generated by
  // running the NeoForge installer. If any is missing we must fall through to
  // the installer fallback below, otherwise mod loading fails with
  ...
  if (isModernNeo) {
    ...
    const neoDir = path.join(librariesDir, 'net', 'neoforged', 'neoforge', versionSuffix || '')
    const neoClientJar = versionSuffix ? path.join(neoDir, `neoforge-${versionSuffix}-client.jar`) : null
    const jarOk = p => p && fs.existsSync(p) && fs.statSync(p).size > 0
    if (!jarOk(srgJar) || !jarOk(extraJar) || !jarOk(neoClientJar)) {
      onProgress?.({ phase: 'loader_patch', log: 'NeoForge client jars missing — running the installer to generate them.', done: 1, total: 1 })
      return null
    }
  }

  let customClientJar = null
  if (needsClientJars && !isModernNeo && installerPath) {
...
module.exports = {
  ...
  readVersionJsonFromInstance,
  readInstallProfileFromInstance,
  readInstallProfile,
