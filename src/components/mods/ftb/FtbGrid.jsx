...
import FtbCard from './FtbCard'

export default function FtbGrid({ results, loading, error, view, onSelect, hasMore, onLoadMore }) {
  if (error) {
    return (
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-1 overflow-y-auto pr-1" style={{ scrollbarColor: 'rgba(255,255,255,0.10) transparent' }}>
        {view === 'grid' ? (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-2.5 p-1">
            {results.map(p => (
              <FtbCard key={p.project_id} project={p} view="grid" onClick={onSelect} />
            ))}
          ...
          <div className="flex flex-col gap-1 p-1">
            {results.map(p => (
              <FtbCard key={p.project_id} project={p} view="list" onClick={onSelect} />
            ))}
