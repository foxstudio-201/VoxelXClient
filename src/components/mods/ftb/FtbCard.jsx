...
function GridCard({ project, onClick }) {
  const tagBadges = (project.categories || [])
    .filter(c => !c.match(/^\d/) && c !== 'FTB')
    .slice(0, 2)
  ...
  return (
    <button
      ...
      style={{ border: '1px solid rgba(255,255,255,0.08)', minHeight: 110 }}
      onMouseEnter={e => {
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden rounded-xl">
        {project.icon_url ? (
          <>
            <img src={project.icon_url} alt="" className="absolute right-0 top-0 h-full"
              style={{ width: '60%', objectFit: 'cover', filter: 'blur(6px)', opacity: 0.8, transform: 'scale(1.1)', transformOrigin: 'right center' }}
              loading="lazy" />
      ...
      <div className="relative z-10 p-3.5 flex gap-3 h-full">
        <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 bg-black/30 flex items-center justify-center ring-1 ring-white/10">
          {project.icon_url
            ...
            : <svg className="w-7 h-7 text-white/15" fill="currentColor" viewBox="0 0 24 24"><path d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
          }
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-between">
          <div>
            ...
            <p className="text-white/55 text-xs leading-relaxed line-clamp-2">
              {project.description || <span className="text-white/25 italic">FTB Modpack</span>}
            </p>
          </div>
          ...
          <div className="flex items-center justify-between gap-2 mt-2">
            <div className="flex flex-wrap gap-1">
              {tagBadges.map(t => (
                <span key={t} className={`text-[10px] px-2 py-0.5 rounded-full font-semibold capitalize ${TAG_COLORS[t] || 'bg-white/10 text-white/40'}`}>
                  {t}
                </span>
              ))}
            </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {tagBadges.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {tagBadges.map(t => (
              <span key={t} className={`text-[10px] px-2 py-0.5 rounded-full font-semibold capitalize ${TAG_COLORS[t] || 'bg-white/10 text-white/40'}`}>{t}</span>
            ))}
          </div>
        )}
      </div>
...
export default function FtbCard({ project, view = 'grid', onClick }) {
