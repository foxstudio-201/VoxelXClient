...
export default function CurseForgeGrid({ results, loading, error, view, onSelect, hasMore, onLoadMore }) {
  if (error) {
    return (
      ...
      >
        {view === 'grid' ? (
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-2.5 p-1">
            {results.map(p => (
              <CurseForgeCard key={p.project_id} project={p} view="grid" onClick={onSelect} />
            ))}
          ...
          <div className="flex flex-col gap-1 p-1">
            {results.map(p => (
              <CurseForgeCard key={p.project_id} project={p} view="list" onClick={onSelect} />
            ))}
