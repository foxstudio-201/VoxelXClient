...
export default function ServerConsole({ server, onBack }) {
  ...
  const statusColor = { online: 'text-orange-400', starting: 'text-yellow-400', offline: 'text-white/30' }[status] || 'text-white/30'
  const statusDot = { online: 'bg-orange-400 animate-pulse', starting: 'bg-yellow-400 animate-pulse', offline: 'bg-white/20' }[status] || 'bg-white/20'
          ...
          {activeTab === 'console' && (
            <>
              ...
              <button onClick={() => { navigator.clipboard.writeText(logs.join('\n')).then(() => { setCopied(true); setTimeout(() => setCopied(false), 2000) }) }}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs transition-all ${copied ? 'bg-orange-500/15 text-orange-400' : 'text-white/35 hover:text-white/60 hover:bg-white/5'}`}>
                {copied
              ...
              {(downloading || dlProgress) && (
                <div className="flex-shrink-0 px-3 py-2 border-b border-white/5 bg-black/20">
                  ...
                  <div className="w-full h-1 bg-white/8 rounded-full overflow-hidden">
                    <div className="h-full bg-orange-400 rounded-full transition-all" style={{ width: `${dlProgress?.percent ?? 0}%` }} />
                  </div>
                </div>
              )}
      ...
      <div className="w-64 flex-shrink-0 flex flex-col overflow-hidden bg-black/15">
          ...
          <div className="flex gap-2 mt-3">
            {!isRunning ? (
              <button onClick={handleStart} disabled={downloading}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-400 text-white text-sm font-bold transition-all active:scale-95 disabled:opacity-50">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ scrollbarColor: 'rgba(255,255,255,0.1) transparent' }}>
          ...
          <div>
            ...
            <div className="w-full h-2 bg-white/8 rounded-full overflow-hidden">
              <div className="h-full bg-orange-400/60 rounded-full transition-all duration-1000"
  ...
  return (
    <div className="relative">
        ...
        <div ref={sugRef}
          ...
          {suggestions.map((s, i) => (
            <button key={s.cmd} type="button"
              ...
              className={`w-full flex items-center gap-3 px-3 py-2 text-left transition-all ${i === selIdx ? 'bg-orange-500/15' : 'hover:bg-white/5'}`}>
              <span className={`text-xs font-mono font-bold flex-shrink-0 ${i === selIdx ? 'text-orange-400' : 'text-white/70'}`}>
                /{s.cmd}
              </span>
      ...
      <div className="flex items-center gap-2 px-3 py-2">
        <span className="text-orange-400/60 font-mono text-xs flex-shrink-0">/</span>
        <input
          ref={inputRef}
          value={value}
        ...
        <button type="button" onClick={onSubmit} disabled={disabled || !value.trim()}
          className="px-3 py-1 rounded-lg bg-orange-500/15 text-orange-400 text-xs font-semibold hover:bg-orange-500/25 transition-all disabled:opacity-30">
          {t('server.console.sendBtn')}
        </button>
        <button type="button" onClick={onToggleScroll}
          className={`px-2 py-1 rounded-lg text-xs transition-all ${autoScroll ? 'bg-orange-500/15 text-orange-400' : 'text-white/25 hover:text-white/50'}`}>
          ↓
        </button>
      </div>
    </div>
...
function IpDisplay({ tunnelAddr, tunnelStatus, server }) {
  ...
  return (
    <div className={`rounded-xl px-3 py-2.5 border ${accent ? 'border-orange-500/25 bg-orange-500/5' : 'border-white/8 bg-white/3'}`}>
      <div className="flex items-center justify-between mb-1">
        <span className={`text-[10px] font-semibold uppercase tracking-wider ${accent ? 'text-orange-400/70' : 'text-white/35'}`}>
          {label}
        </span>
        {accent && (
          <span className="flex items-center gap-1 text-[9px] text-orange-400/60">
      ...
      <div className="flex items-center justify-between gap-1.5">
        <span className={`text-sm font-bold font-mono truncate transition-all select-none ${
          visible ? accent ? 'text-orange-400' : 'text-white/80' : 'blur-[3px] text-white/40'
        }`}>
          ...
          {visible && (
            <button onClick={copy}
              className={`flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-semibold transition-all ${
                copied ? 'bg-orange-500/20 text-orange-400' : 'bg-white/8 text-white/40 hover:text-white/70 hover:bg-white/12'
              }`}>
              {copied
